...
<div className="mb-3">
  ...
  <textarea className="form-control" id="exampleFormControlTextarea1" rows="8" value={text} onChange={handleOnChange}
   style={{backgroundColor: props.mode==='dark' ? 'grey' : 'white', color: props.mode==='dark' ? 'white' : 'black'}}></textarea>
    ...
    <button disabled={text.length===0} className="btn btn-primary mx-3 my-3 " onClick={handleDownClick}>LowerCase</button>
    <button disabled={text.length===0} type="button" className="btn btn-secondary mx-4" onClick={handleDelete}>Delete Content</button>
